...
- ⚠️ **Cần kiểm chứng khi implement:** field/format chính xác của hook output (`additionalContext` cho `SessionStart`/`PostToolUse`) theo docs Claude Code hiện hành — API hooks có thể đã đổi. Cô lập toàn bộ giả định này trong `integrations/claude-code/`.
