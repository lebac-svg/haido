...
- **v0.2.3 — Trạm review** (đề xuất được user chốt 11/07): ✅ **ĐÃ GIAO 11/07/2026** — dải báo động viền vàng chỉ xuất hiện khi có ghi chú needs_review: mỗi mục gồm anchor drift/missing, **diff token màu** (`⟨- cũ⟩` đỏ / `⟨+ mới⟩` xanh — cùng engine tokenDiff của `haido stale`) và lệnh `haido reanchor … --confirm` kèm nút chép; click tiêu đề là spotlight ghi chú trên cả hai bản đồ. Diff tính server-side, snapshot thô không bao giờ ship xuống trang.
