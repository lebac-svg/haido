...
- **v0.2 — Bản đồ tri thức** (`haido viz`): ✅ **ĐÃ GIAO SỚM 11/07/2026** — 1 file HTML tự chứa, canvas 2D force layout theo import/co-change, node màu theo thư mục (palette đã validate), vòng trắng = có ghi chú, vòng vàng ⚠ = cần review, hover tooltip + click panel + search + lọc; `haido viz [--out] [--open]`. (Chế độ treemap: để sau nếu dogfood cần.)
- **v0.2.1 — Bản đồ sống** (`haido viz --live [--port]`): ✅ **ĐÃ GIAO 11/07/2026** — serve trang viz trên `127.0.0.1` + SSE: file mới nở ra trên bản đồ theo thời gian thực, file đang được sửa phát sáng rồi nguội dần (~8s), ghi chú drift chớp vàng ngay khi code lệch neo; nguồn thay đổi = watcher (code save) + poll `PRAGMA data_version` (memory ghi từ MCP/terminal khác). Chi tiết: ARCHITECTURE §11b.
- **v0.2.2 — Đài chỉ huy** (feedback user 11/07: "chia thành nhiều khung"): ✅ **ĐÃ GIAO 11/07/2026** — địa cầu 3D là khung lớn nhất (tương tác: kéo xoay — không tự quay, lăn dolly, click chọn, tooltip kỹ thuật, spotlight dây liên kết + dây neo y như 2D), hải đồ 2D cỡ làm việc (tự căn khung tới khi người dùng cầm lái), băng "đang diễn ra" (sự kiện live có timestamp, ⚡ agent / ✎ người), khung soi chi tiết (cấu trúc symbol L-range + ghi chú neo + liên kết) và nhật ký hải trình (mọi ghi chú theo dòng thời gian). Payload thêm `files[].syms` + `memories[].created` (additive). Checkbox "toàn hải đồ" thu về bản đồ toàn màn hình.
- **v0.2.3 — Trạm review + recall hiển thị** (đề xuất được user chốt 11/07): ✅ **ĐÃ GIAO 11/07/2026** — cột phải thường trực (rộng ≈ nửa nhật ký): biển lặng hiện `✅ mọi ghi chú đều khớp code` + bộ đếm `🤖 tiêm cho agent: N`; có drift thì viền hổ phách + thẻ gồm anchor drift/missing, **diff token màu** (`⟨- cũ⟩` đỏ / `⟨+ mới⟩` xanh — cùng engine tokenDiff của `haido stale`, tính server-side, snapshot thô không ship xuống trang) và lệnh `haido reanchor … --confirm` kèm nút chép; click tiêu đề là spotlight trên cả hai bản đồ. **Recall thành nhìn-thấy-được:** hook đóng dấu `lastInject` mỗi lần tiêm ghi chú cho agent → server phát `hot.injected` (poll session states) → kim cương nhấp lam + băng sự kiện ghi `🤖 tiêm ghi chú: <tên>` — bằng chứng trực quan trí nhớ đang được dùng.
- **v0.2 — Stop-hook reflection:** ✅ **ĐÃ GIAO 11/07/2026** — hook `Stop` chặn MỘT lần khi phiên sửa ≥2 file mà chưa ghi memory nào, nhắc agent tự vấn *"có quyết định/bẫy nào đáng ghi không?"*; ba lớp chống lặp (stop_hook_active, cờ một-lần-mỗi-phiên, đếm memory đã ghi). Kèm SessionStart(source=compact) tự re-brief bản đồ + reset dedup sau khi context bị nén. (Reflection có kỷ luật, phục vụ việc ghi; KHÔNG phải để LLM tự sinh/tự sửa memory không bằng chứng — vẫn là non-goal §6.)
