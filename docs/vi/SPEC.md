...
- **v0.2.2 — Đài chỉ huy** (feedback user 11/07: "chia thành nhiều khung"): ✅ **ĐÃ GIAO 11/07/2026** — hải đồ 2D là nhân vật chính, xung quanh là khí cụ: địa cầu 3D tự xoay (cùng một mô phỏng, phép chiếu thứ hai, xoay trong mặt phẳng của đám mây), băng "đang diễn ra" (sự kiện live có timestamp, ⚡ agent / ✎ người), khung soi chi tiết (cấu trúc symbol L-range + ghi chú neo + liên kết) và nhật ký hải trình (mọi ghi chú theo dòng thời gian). Payload thêm `files[].syms` + `memories[].created` (additive). Checkbox "toàn hải đồ" thu về bản đồ toàn màn hình.
